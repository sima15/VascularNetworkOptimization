...
public abstract class LocatedAgent extends ActiveAgent implements Cloneable {
    ...
    //Added Delin, to provide a noskin option in the bottom of the grid.
	private int noSkinBottomLayerBoundary = 0;
...
//		}
    ...
    public void initFromProtocolFile(Simulator aSim, XMLParser aSpeciesRoot) {
        ...
        //Added by Delin, to give the noSkin option to the bottom of simulation
        List<Element> params = aSpeciesRoot.getChildren("param");
        for(Element param: params){
        	if(param.getAttribute("name").equals("noSkinBottomLayerBoundary")){
        		noSkinBottomLayerBoundary = Integer.parseInt(param.getValue());
        	}
        }
    }
...
//            }
    ...
    protected boolean hasOpenBoundary() {
        ...
        if (_location.x < 30)
        //MD Delin, to control this option from protocol file
        if (_location.x < noSkinBottomLayerBoundary)
            return false;
