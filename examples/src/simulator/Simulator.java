...
public class Simulator {
	...
	// added by Chris Johnson so that simulations can be restarted with solutes
	// in place
	...
	// public static String output2 = "Iter;" + "AllVEGF;" + "NumberofCells;" +
	// "VEGFperCell";
	// public static String output3 = "";
	/** agentTimeStep */
	// to make it available to AgentContainer class and at the same time be
	// readable from the simulator
	...
	// If true, all reaction-diffusion solvers will be solved at each agent time
	// step
	// This will slow simulations down, I've added it so that it behaves more
	// like Biocellion
	...
	public void SimulatorInitializer(String protocolFile, String resultPath, boolean writeEnvOutput, boolean writePOV) {
		try {
			...
			lateBindings();
			// bvm 27.1.2009 moved this from above to allow better povray
			// outputs
	...
	private void lateBindings() throws Exception {
		...
		for (Species spec : speciesList) {
			for (Species spec2 : speciesList) {
				if (!spec.speciesName.equalsIgnoreCase(spec2.speciesName)
						&& !spec.switchingLags.containsKey(spec2.speciesName)) {
	...
	public void step() {
		...
		if (((SimTimer.getCurrentTime() - _lastOutput) >= (_outputPeriod - 0.01 * SimTimer.getCurrentTimeStep()))
				|| SimTimer.simIsFinished())
			writeReport();

		// sonia 26.04.2010
		// only remove the agents from the system after recording all the
	...
	public void updateChart() {
		...
		for (int iSolute = 0; iSolute < nSolute; iSolute++)
			_graphics.updateChart(0, iSolute, SimTimer.getCurrentTime(), world.getBulk("tank").getValue(iSolute));

		for (int iSpecies = 0; iSpecies < nSpecies; iSpecies++)
			_graphics.updateChart(1, iSpecies, SimTimer.getCurrentTime(), speciesList.get(iSpecies).getPopulation());
	...
	public void createSimulator() {
		...
		// sonia: read the flag from protocol file to decide if this is a
		// chemostat run
		...
		// sonia 05.2011 bug fix: the code was not finding the random.state file
		// because no path was
		...
		if (randomFile.exists()) {
			 ...
			 * is initialised using this file. this ensures that the random
			 * number stream does not overlap when running repeated simulations
			 * with the same protocol file Chinmay 11/08/2009
			 */
			...
			try {
				...
				LogFile.writeLog(
						"Simulator.createSimulator() : error met while reading in random number state file" + e);
				System.exit(-1);
			} finally {
				try {
			...
			// Chinmay - added MTRandom.java to utils and changed the RNG to use
			// that class instead. 11/08/2009
		...
		if (localRoot.getParamBool("restartPreviousRun")) {
			...
			simTimer.setTimerState(_resultPath + File.separator + "lastIter" + File.separator + "env_Sum(last).xml");
		}
	...
	public void createSoluteCompositions() {
		...
		if (soluteCompositionsMarkUp.get_localRoot() != null) {
			...
			soluteCompositionList = new SoluteComposition[0];
	...
	public void createSolvers() throws Exception {
		...
		for (Element aSolverMarkUp : _protocolFile.buildSetMarkUp("solver")) {
			...
			DiffusionSolver aSolver = (DiffusionSolver) parser.instanceCreator("simulator.diffusionSolver");
			aSolver.init(this, parser);
			aSolver.register();
			// System.out.println("\t\t" + aSolver.solverName + " (" +
			// aSolver.solverIndex + ")");
		}
	...
	public void createSpecies() throws Exception {
		try {
			...
			for (Element aSpeciesMarkUp : _protocolFile.buildSetMarkUp("species")) {
				...
				// sonia: creating a list with the plasmid names which will be
				// used afterwards to write the agentSum report
			}
			// System.out.print("\t done\n");
			...
			System.out.println("Total species count: " + speciesList.size());
			if (useAgentFile) {
				System.out.println("Using last agent state xml");
				recreateSpecies();

				// sonia: 20-07-09
				// I've added the line of code below, so that it is possible to
				// create new agents from the protocol file when restarting
				// from a previous simulation (environment) using a new protocol
				// file
				checkAgentBirth();
	...
	public void recreateSpecies() throws Exception {
		...
		System.out.println("Recreating Species: " + simulationRoot.buildSetMarkUp("species").size());
		for (Element aSpeciesMarkUp : simulationRoot.buildSetMarkUp("species")) {
			...
			// check consistency with protocol file declarations
			boolean isConsistent = (speciesList.get(counterSpecies).speciesName
					.equals(aSpeciesRoot.getAttributeStr("name")));
		
			if (!isConsistent)
				throw new Exception("Agent input file is inconsistent with protocol file: ");

			System.out.println("consistent : " + aSpeciesRoot.getElement().getText().length());
			if (!(aSpeciesRoot.getElement().getText().length()<=1)) {
				// Process agents description
				String dataSource = aSpeciesRoot.getElement().getText();
				String[] allAgentData = dataSource.split(";\n");
				progenitor = speciesList.get(spIndex).getProgenitor();
				System.out.println("Data length" + allAgentData.length);

				// don't use the last index because it is a string with only ']'
				for (int i = 0; i < allAgentData.length; i++)
				...
				LogFile.writeLog(speciesList.get(counterSpecies).speciesName + " : "
						+ speciesList.get(counterSpecies).getPopulation() + " agents created from input file.");
				System.out.println(speciesList.get(counterSpecies).speciesName + " : "
						+ speciesList.get(counterSpecies).getPopulation() + " agents created from input file.");
			}
			System.out.println(counterSpecies);
			counterSpecies++;
		}
	}
	...
	public void writeReactionFrequencyReport() {
		try {
			...
			for (int i = 0; i < molecularReactionList.length; i++) {
				...
				output.append(reac + "\t" + molecularReactionList[i].reactionName + "\t"
						+ molecularReactionList[i].numOccurence + "\n");
			}
	...
	public void writeReport() {
		...
		if (writeEnvOutput) {
			...
			try {
				...
				LogFile.writeError("System description of grids failed:" + e.getMessage(), "Simulator.writeReport()");
			}
		}
		...
		try {
			...
			LogFile.writeError("System description of agents failed:" + e.getMessage(), "Simulator.writeReport()");
		}
	...
	private void writeAgar() {
		int currentIter = SimTimer.getCurrentIter();
		try {
			FileOutputStream fs = new FileOutputStream(_resultPath + File.separator + "SoluteConcentration"
					+ File.separator + "Agar " + LogFile.numberToString(currentIter) + ".txt");
			String output = "";
	...
	// output += '\n';
	// }
	// File f = new File(_resultPath + File.separator + "SoluteConcentration" +
	// File.separator + i + File.separator + fileName + " solute " +
	// LogFile.numberToString(currentIter) + ".txt");
	// f.getParentFile().mkdir();
	// FileWriter fs = new FileWriter(f);
	//
	// if (fileName.endsWith("VEGF")) {
	//
	// FileWriter fs2 = new FileWriter(_resultPath + File.separator +
	// "Concentration_Per_cell" + File.separator + fileName +
	// "Allsolute_per_cell" + ".txt");
	//// FileWriter fs3=new
	// FileWriter(_resultPath+File.separator+"Concentration_Per_cell"+File.separator+
	// fileName +" solute per cell "+
	// LogFile.numberToString(currentIter)+".txt");
	// FileWriter fs3 = new FileWriter(_resultPath + File.separator +
	// "Concentration_Per_cell" + File.separator + fileName +
	// "Allsolute_per_cellDetailed" + ".txt");
	//
	// if (currentIter != 0 && agentGrid.species_Name.endsWith("Patch00")) {
	//
	// output2 += currentIter + "\t" + TotalSolute + "\t" +
	// agentGrid.species_number + "\t" + TotalSolute /
	// (agentGrid.species_number);
	// }
	...
	private static int findSoluteIndexByName(String name, SoluteGrid[] sgs) {
		for (int i = 0; i < sgs.length; i++) {
			if (sgs[i].getName().equalsIgnoreCase(name))
				return i;
		}
		return -1;
	}
	...
	/**
	 * Check whether we will initialize from agent and bulk input files If the
	 * "restartPreviousRun" param is true, this will also set the correct files
	 * for reading in the last state
	 *
	 * @param protocolFile
	 */
	public void detectInputs(String protocolFile) {
		...
		if (restartInfo.getParamBool("restartPreviousRun")) {
			...
			agentFile = new XMLParser(
					_resultPath + File.separator + "lastIter" + File.separator + "agent_State(last).xml");
			bulkFile = new XMLParser(_resultPath + File.separator + "lastIter" + File.separator + "env_Sum(last).xml");

			LogFile.writeLog("Restarting run from previous state in directory: " + _resultPath);
		...
		if (useAgentFile) {
			...
			// agentFileName = protocolFile.subSequence(0, index + 1) +
			// agentFileName;
